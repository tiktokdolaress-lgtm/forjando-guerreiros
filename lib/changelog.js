...
export const CURRENT_APP_VERSION = 'v1.4.2';
...
export const CHANGELOG_RELEASES = [
  {
    ...
    title: {
      pt: 'Cronômetro de Precisão ao Iniciar e Cair · Retenção Seminal Oficial',
      en: 'Precision Stopwatch on Start & Fall · Official Semen Retention',
      es: 'Cronómetro de Precisión al Iniciar y Caer · Retención Seminal Oficial',
    },
    badge: 'latest',
    isLatest: true,
    highlights: [
      {
        icon: '⏱️',
        title: {
          pt: 'Cronômetro de Precisão com Hora e Minuto',
          en: 'Precision Stopwatch with Exact Hour & Minute',
          es: 'Cronómetro de Precisión con Hora y Minuto Exactos',
        },
        desc: {
          pt: 'Ao entrar no app ou ao registrar uma queda, você define a hora exata. O cronômetro conta segundo a segundo em tempo real a partir deste instante exato.',
          en: 'When onboarding or registering a fall, you choose the exact time. The stopwatch counts second-by-second in real-time from this precise moment.',
          es: 'Al registrarse o registrar una caída, defines la hora exacta. El cronómetro cuenta segundo a segundo en tiempo real desde este momento preciso.',
        },
      },
      {
        icon: '🔥',
      ...
      },
      {
        icon: '⚙️',
        title: {
          pt: 'Painel de Calibração em Configurações',
          en: 'Calibration Panel in Settings',
          es: 'Panel de Calibración en Ajustes',
        },
        desc: {
          pt: 'Possibilidade de recalibrar a qualquer momento a data e horário exatos de início da Retenção Seminal, Sem Pornô e Sem Masturbação.',
          en: 'Ability to recalibrate at any time the exact start date and time for Semen Retention, Porn-Free, and Masturbation-Free pillars.',
          es: 'Posibilidad de recalibrar en cualquier momento la fecha y hora exacta de inicio de la Retención Seminal, Sin Porno y Sin Masturbación.',
        },
      },
    ],
  },
...
export async function dispatchUpdateNotification(lang = 'pt') {
  if (typeof window === 'undefined') return;
  try {
    const { localNotify } = await import('./notify');
    const titles = {
      pt: `⚔️ Decretos da Forja · ${CURRENT_APP_VERSION}`,
      en: `⚔️ Forge Decrees · ${CURRENT_APP_VERSION}`,
      es: `⚔️ Decretos de la Forja · ${CURRENT_APP_VERSION}`,
    };
    const bodies = {
      pt: 'Novas melhorias e traduções ativas no app. Toque para conferir.',
      en: 'New upgrades and translations active in the app. Tap to review.',
      es: 'Nuevas mejoras y traducciones activas en la app. Toca para consultar.',
    };
    const l = (lang === 'en' || lang === 'es') ? lang : 'pt';
    localNotify(titles[l], bodies[l], `fg-update-${CURRENT_APP_VERSION}`);
  } catch (e) {}
}
