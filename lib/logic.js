...
export const mergeS = (o) => {
  ...
  // Auto-sync: se um projeto estiver arquivado, suas tarefas vinculadas devem estar arquivadas
  if (Array.isArray(merged.projects) && Array.isArray(merged.tasks)) {
    const archivedProjIds = new Set(
      merged.projects.filter((p) => p && p.archived).map((p) => String(p.id))
    );
    if (archivedProjIds.size > 0) {
      merged.tasks.forEach((t) => {
        const pId = t.projectId != null ? t.projectId : t.proj;
        if (pId != null && archivedProjIds.has(String(pId))) {
          t.archived = true;
        }
      });
    }
...
export const isTaskPostponed = (t) => {
  if (!t || t.archived) return false;
  if (isDone(t, today())) return false;
  return Boolean(t.postponedTo || t.postponed);
};
...
export const isTaskOverdue = (t) => {
  ...
  if (isDone(t, today())) return false;
  const tod = today();
  if (t.postponedTo) return t.postponedTo < tod;
  if (t.dueDate) return t.dueDate < tod;
  if ((t.rep || 'unica') === 'unica') {
    if (t.createdAt && t.createdAt < tod) return true;
  }
  return false;
};

export const isProjLate = (S, p) => {
  if (!p || p.archived || p.status === 'concluido' || p.cancelled) return false;
  if (p.deadline && daysBetween(today(), p.deadline) < 0) return true;
  const linked = (S?.tasks || []).filter((t) => (String(t.projectId) === String(p.id) || String(t.proj) === String(p.id)) && !t.archived);
  return linked.some((t) => isTaskOverdue(t) || isTaskPostponed(t));
};

export const repDue = (t, dateStr) => {
  if (!t || t.archived) return false;
  if (t.postponedTo && t.postponedTo > dateStr) return false;
  const r = t.rep || 'unica';
...
export const projPct = (S, p) => {
  const linked = S.tasks.filter((t) => String(t.projectId) === String(p.id) || String(t.proj) === String(p.id));
